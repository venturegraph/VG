...
import { PostForm, InitialPostData } from '@/components/admin/PostForm';
...
export default function AdminEditPostPage() {
  ...
  const [postData, setPostData] = useState<InitialPostData | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      ...
      try {
        ...
        if (!data) {
          setFetchError(`Post with ID "${postId}" was not found or has been deleted.`);
          return;
        }
        ...
        setPostData({
          ...
          status: data.status,
          total_raised: data.total_raised ? String(data.total_raised) : '',
          founded_year: data.founded_year,
          shutdown_year: data.shutdown_year,
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface flex flex-col selection:bg-primary-container selection:text-on-primary">
      ...
      <main className="w-full max-w-[1520px] mx-auto px-4 lg:px-8 py-8 flex-1">
        {isLoading ? (
          <div className="min-h-[400px] flex flex-col items-center justify-center gap-3 text-secondary">
            <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin" />
            <p className="text-xs font-mono">Loading post intelligence data...</p>
          </div>
        ) : fetchError ? (
          <div className="p-8 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 text-center max-w-lg mx-auto space-y-4 shadow-sm">
            <div className="w-12 h-12 rounded-full bg-error/10 text-error flex items-center justify-center mx-auto">
              <span className="material-symbols-outlined text-2xl">search_off</span>
            </div>
            <div>
              <h2 className="text-lg font-bold text-on-surface">Post Not Found</h2>
              <p className="text-xs text-secondary mt-1">{fetchError}</p>
            </div>
            <Link
              href="/admin/posts"
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary text-on-primary text-xs font-semibold hover:bg-primary-container transition-colors shadow-xs"
            >
